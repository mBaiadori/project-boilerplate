...
export const eccSeedAgents: AgentDefinition[] = [
  {
    ...
    description: 'Conecta decisões arquiteturais, valida links cruzados no grafo de documentação e garante aderência aos templates de engenharia.',
...
Sempre consulte o grafo com 'docs_get_graph', valide a linguagem ubíqua com 'dictionary_validate_terms' e registre ou atualize termos de domínio canônicos com 'dictionary_upsert_term'.`,
    ...
    tools: ['docs_get_graph', 'docs_read_file', 'docs_propose_diff', 'dictionary_validate_terms', 'dictionary_upsert_term', 'templates_get_template'],
  ...
  },
  {
    ...
    description: 'Audita o repositório em busca de documentos desatualizados (drafts órfãos, status inconsistentes) e verifica se os termos técnicos estão no dicionário.',
    system_prompt: `Você é o Auditor de Conformidade & QA de Documentação.
Seu foco é rigor metodológico: verificar se requisitos contêm critérios de aceitação testáveis, se documentos em 'review' possuem todas as seções obrigatórias e se termos técnicos seguem o padrão da equipe.`,
    skills: ['quality-assurance-gatekeeper', 'living-docs-governance'],
    tools: ['docs_list_by_status', 'docs_read_file', 'dictionary_validate_terms', 'docs_get_graph'],
    category: 'quality',
  ...
  },
  {
    ...
    description: 'Mantém a paridade entre o que a engenharia está codando e o que está documentado nas especificações e guias de API.',
...
Analise trechos de código, APIs e schemas, propondo atualizações nas documentações com 'docs_propose_diff' sempre que houver discrepâncias de implementação.`,
    ...
    tools: ['docs_read_file', 'docs_propose_diff', 'docs_get_graph', 'templates_get_template'],
  ...
  },
  {
    ...
    description: 'Transforma ideias brutas em documentos técnicos detalhados, claros e padronizados com base nos templates da equipe.',
...
Ajude o usuário a redigir especificações técnicas claras, concisas e acionáveis, aplicando os templates oficiais do repositório e cadastrando novos termos canônicos no dicionário com 'dictionary_upsert_term' sempre que novos conceitos forem formalizados.`,
    ...
    tools: ['templates_get_template', 'docs_propose_diff', 'dictionary_validate_terms', 'dictionary_upsert_term', 'docs_read_file'],
    category: 'governance',
    recommended_model: 'gemini-2.5-flash',
    temperature: 0.5,
    source: 'ecc',
