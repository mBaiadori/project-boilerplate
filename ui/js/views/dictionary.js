...
export function initDictionaryView({ onOpenDocument } = {}) {
  ...
  function openTermModal(termObj = null) {
    if (termObj) {
      isCodenameManuallyEdited = true;
  ...
  async function loadDictionary() {
    if (dictTermsTbody) {
      dictTermsTbody.innerHTML = '<tr><td colspan="4" style="padding: 30px; text-align: center; color: #64748b;">Carregando Dicionário Ubíquo...</td></tr>';
    }
    try {
      ...
      if (dictTermsTbody) {
        dictTermsTbody.innerHTML = '<tr><td colspan="4" style="padding: 20px; color: #ef4444; text-align: center;">Erro ao carregar Dicionário Ubíquo.</td></tr>';
      }
    }
  }

  // 6. Renderizar Lista em Tabela
