...
export function initTreeView({ onOpenFile, onWorkspaceChanged }) {
  ...
  const btnConfirmCancelTree = document.getElementById('btn-confirm-cancel-tree');
  const btnConfirmCreateTree = document.getElementById('btn-confirm-create-tree');
  ...
  function computeTargetPath() {
    const rawName = (treeItemName.value || '').trim().toLowerCase().replace(/\s+/g, '-');
    const type = treeItemType.value;
    const cleanName = rawName || 'novo-item';

    let path = '';
    if (customCreationParentFolder) {
      path = type === 'folder' ? `${customCreationParentFolder}/${cleanName}` : `${customCreationParentFolder}/${cleanName}.md`;
  ...
  btnConfirmCreateTree.addEventListener('click', async () => {
    ...
    try {
      const { ok, data } = await API.createProjectFile({ path, is_folder, content: initialContent });
      if (ok && data.success) {
  ...
  if (btnConfirmRename) {
    btnConfirmRename.addEventListener('click', async () => {
      ...
      try {
        const { ok, data } = await API.renameProjectFile({ old_path, new_path });
        if (ok && data.success) {
          closeRenameModal();
          cachedTreeNodes = data.tree || [];
          renderTree(cachedTreeNodes);
  ...
  async function loadDocumentTree() {
    treeContainer.innerHTML = '<div class="loading-state">Carregando documentos...</div>';
    try {
      const data = await API.getProjectTree();
      cachedTreeNodes = data.tree || [];
      renderTree(cachedTreeNodes);
    } catch (err) {
      treeContainer.innerHTML = '<div class="empty-state">Erro ao carregar estrutura de arquivos.</div>';
    }
  }
  ...
  function renderTree(nodes = []) {
    ...
    let domainNodes = [];
    const domainFolder = nodes.find(n => (n.name === 'domains' || n.path === 'domains') && n.type === 'dir');
    if (domainFolder && domainFolder.children) {
      ...
      domainNodes = nodes.filter(n => {
        ...
        return !p.startsWith('templates') && !p.startsWith('engenharia') && !p.startsWith('patterns') && !p.startsWith('.github') && p !== 'index.md';
      });
    }
    ...
    if (!domainNodes || domainNodes.length === 0) {
      treeContainer.innerHTML = `
        <div class="tree-empty-state" style="display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 28px 14px; gap: 8px; width: 100%; box-sizing: border-box;">
          ...
          <strong style="font-size: 13px; font-weight: 600; color: var(--text-normal); margin: 0;">Nenhum domínio cadastrado</strong>
          ...
          <button class="btn btn-primary btn-sm" style="margin-top: 6px; width: 100%; font-size: 11.5px; padding: 6px 10px; display: inline-flex; align-items: center; justify-content: center; gap: 4px;" onclick="document.getElementById('btn-tree-new-file').click()">
            <span>+ Novo Domínio / Feature</span>
          </button>
        </div>
      `;
      return;
  ...
  window.treeAddInside = function(dirPath) {
    ...
    if (panel && nameInput) {
      ...
      document.getElementById('tree-item-path-preview').innerHTML = `Destino: <code>${dirPath}/novo-arquivo.md</code>`;
      nameInput.focus();
    }
  };
  ...
  window.treeDeletePath = async function(path) {
    if (confirm(`Tem certeza que deseja excluir "${path}"? Esta alteração será registrada no workspace e consolidada no PR.`)) {
      try {
